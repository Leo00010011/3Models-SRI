...
public static class Extension
{
    ...
    public static double Scalar_Mult<K>(this ISRIVector<K, double> item1, ISRIVector<K, double> item2) where K: notnull => item1.GetKeys().Sum(x => item1[x] * item2[x]);
    ...
    public static double Norma2<K>(this ISRIVector<K, double> item) where K: notnull => Math.Pow(item.Sum(x => Math.Pow(x, 2)), 0.5);
}