...
public static class Extension
{
    ...
    public static double Scalar_Mult<K>(this Dictionary<K, double> item1, Dictionary<K, double> item2) where K: notnull => item1.Sum(x => item1[x.Key] * item2[x.Key]);
    ...
    public static double Norma2<K>(this Dictionary<K, double> item) where K: notnull => Math.Pow(item.Sum(x => Math.Pow(x.Value, 2)), 0.5);
}